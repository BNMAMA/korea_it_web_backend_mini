...
public class MailService {
    ...
    public ApiRespDto<?> sendMail(SendMailReqDto sendMailReqDto, PrincipalUser principalUser) {
        ...
        if (optionalUser.isEmpty()) {
            return new ApiRespDto<>("faild", "존재하지 않는 이메일입니다.", null);
        ...
        message.setText("링크를 클릭하여 인증을 완료해주세요 " +
                "http://localhost:8080/mail/verity?verifyToken=" + verifyToken);
    ...
    public Map<String, Object> verify(String token) {
        Claims claims = null; //JWT토큰의 핵심 데이터 컨테이너, (Payload- 실제로 전달하려는 데이터가 담기는 곳)

        Map<String, Object> resultMap = null; //resultMap = DB 컬럼명과 Java 객체 필드명이 다를 때 매핑을 맞춰줌
        ...
        try {
            ...
            if (optionalUserRole.isEmpty()) {
                resultMap = Map.of("status", "failed", "message", "이미 인증이 완료된 계정입니다");

            } else {
                userRoleRepository.UpdateRoleId(optionalUserRole.get().getUserRoleId(), userId);
                //Optional<UserRole> -> UserRole객체를 꺼내 -> userRoleId를 가지고 오기
